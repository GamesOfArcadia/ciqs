"""
Page_Curve_Tested_on_Hardware_Entropy_rises_Correlations_do_not_v2
Jessy Pensédent | 2026 | IBM Heron r2 | N_BH=6 | Page time t=3
...
try:
    ...
    print("ERROR: CIQA.py not found."); sys.exit(1)
try:
    from CIQS_pipeline import CIQS_pipeline  # noqa: F401
    from CIQM import CouplingMap
except ImportError:
    print("ERROR: CIQS_pipeline.py / CIQM.py not found. (downloadable for free though)"); sys.exit(1)
try:
...
# Set to a job ID to skip execution and replay post-processing only without QPU usage. (USE_SIMULATOR must be false)
# Set back to "" to run a fresh job.
REPLAY_JOB_ID  = ""
